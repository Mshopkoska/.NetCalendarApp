...
namespace CALENDAR_Version_3._0.Services.ReminderEmails.Jobs
{
    public class EventReminderEmailJob : IJob
    {
        ...
        public EventReminderEmailJob(UserManager<ApplicationUser> userManager, IEmailSender sender, ILogger<EmailSender> logger, IDAL _dal)
        {
                                   ...
                                   //imaat datum za Reminder deneshen den ili se sluchuvaat denes i gi izminuvam i prakjam soodveten mail
        {
            ...
            List<Event> emailNotifications = _dal.GetEventsFromDate(current, "notification"); //list of all events that occur today and email notification needs to be sent
            List<Event> emailReminders = _dal.GetEventsFromDate(current, "reminder"); //list of all events that need email reminder to be sent
